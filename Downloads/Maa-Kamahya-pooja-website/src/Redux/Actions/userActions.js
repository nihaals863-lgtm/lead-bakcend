...
export const userRegistergoogle = (regData) => async (dispatch) => {
    try {
        dispatch({
            type: REGISTER_REQUESTS,
        })
        ...
        const { data } = await axios.post(`${baseUrl}/register`, regData, config)
...
export const userRegisterfb = (regData) => async (dispatch) => {
    try {
        dispatch({
            type: REGISTER_REQUESTS,
        })
